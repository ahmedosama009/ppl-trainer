...
/* توزيع المجموعات الزيادة — مرة واحدة لكل أسبوع على الدورة كلها.
   ...
   قاعدتين:
   ١) كل عضلة بتاخد بالظبط فرقها عن هدف الأسبوع.
   ٢) الجلسات الثقيلة (A) سقفها +٢ مجموعة فوق أسبوع ١ — التقدّم فيها
      بالوزن مش بعدد المجموعات. الباقي بيروح لجلسات الحجم (B)،
      لأن تكلفة المجموعة في مدى ١٢–١٥ أقل بكتير منها في ٤–٦. */
const HEAVY_CAP = 2;
const _bonusCache = {};
function weekBonus(week) {
  ...
  /* تمارين كل عضلة: جلسات الحجم الأول، وبعدين الثقيلة */
  const slots = {};
  ORDER.forEach(k => {
    const D = sessByKey(k);
    [].concat(D.ex || [], D.absEx || []).forEach((id, i) => {
      const e = EX[id]; if (!e || e.nc || !VOLUME_TARGET[e.m[0]]) return;
      (slots[e.m[0]] = slots[e.m[0]] || []).push({ key: k + '|' + i, sess: k, heavy: !!D.heavy });
    });
  });
  Object.values(slots).forEach(a => a.sort((x, y) => (x.heavy ? 1 : 0) - (y.heavy ? 1 : 0)));

  const base = baseVolume(), added = {};
  Object.keys(VOLUME_TARGET).forEach(m => {
    let need = VOLUME_TARGET[m][week - 1] - (base[m] || 0);
    const list = slots[m] || [];
    if (need <= 0 || !list.length) return;
    let n = 0, guard = 0;
    while (need > 0 && guard++ < 400) {
      const sl = list[n % list.length]; n++;
      if (sl.heavy && (added[sl.sess] || 0) >= HEAVY_CAP) {
        /* لو كل الخانات المتاحة للعضلة دي اتقفلت، نسيب الباقي */
        if (list.every(x => x.heavy && (added[x.sess] || 0) >= HEAVY_CAP)) break;
        continue;
      }
      bonus[sl.key] = (bonus[sl.key] || 0) + 1;
      added[sl.sess] = (added[sl.sess] || 0) + 1;
      need--;
    }
  });
...
/* المدة المقدَّرة للأسبوع الحالي: تسخين + (مجموعات × (راحة + شغل)) + كارديو */
function sessionMinutes(k, week) {
  const D = sessByKey(k);
  let sec = 0;
  sessionPlan(k, week).forEach(p => { sec += p.sets * (EX[p.id].r + 40); });
  const warm = (D.ex || []).length ? 8 : 3;
  const cardio = D.cardio ? D.cardio.min : D.intervals ? 40 : D.steady ? 30 : 0;
  return Math.round((sec / 60 + warm + cardio) / 5) * 5;
}
...
function viewToday() {
  ...
  h += `<div class="card">
    <div class="dayhead">
      ...
      <p>${esc(D.sub)}</p>
      <div class="chips">
        <span class="chip g">المدة ~${L(sessionMinutes(D.k, wk))} دقيقة</span>
  ...
  ORDER.forEach((k, i) => {
    ...
    h += `<button class="exhead" data-jump="${i}" style="border-bottom:1px solid var(--line)">
      ...
      <span class="exmeta">
        ...
        <div class="sub">${n ? L(n) + ' تمرين · ' : ''}~${L(sessionMinutes(D.k, S.week))} دقيقة${D.absEx ? ' · بطن ' + L(D.absEx.length) : ''}${D.cardio ? ' · إرجومتر ' + L(D.cardio.min) + ' د' : ''}</div>
      </span>
